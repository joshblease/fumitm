...
12. **Shell Environment Configuration** (`add_to_shell_config`):
    ...
    - **"Keep last" is re-asserted, not established**: a vendor agent update can append a fresh block *below* fumitm's stub (observed with Aikido's v2 blocks landing in `~/.zlogin`), reverting managed vars. `reassert_shell_stubs()` runs on every `--fix` regardless of tool selection and reports as a `shell-env` ToolResult; `_find_shadowed_stub_files()` detects the shadow at status time via `check_environment_sanity()` (static scan for managed-var exports after the stub's end marker), since the failure otherwise surfaces hours later as a misleading auth/TLS error.
